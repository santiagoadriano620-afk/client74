...
function bindKeys()
	...
	g_keyboard.bindKeyPress("Escape", function ()
		if g_clock.millis() < lastAction + 50 then
			return
		end
...
function save()
	local settings = {
		splitterMarginBottom = bottomSplitter:getMarginBottom()
	}
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
	...
	if useThing then
		...
		if g_game.getFeature(GameBrowseField) and useThing:getPosition().x ~= 65535 then
	...
	if creatureThing then
		...
		if creatureThing:isLocalPlayer() then
			...
			shortcut = not classic and not g_app.isMobile() and "(Alt)" or nil
	...
	if g_game.getFeature(GameBot) and useThing and useThing:isItem() then
		...
		if useThing:getSubType() > 1 then
			menu:addOption("ID: " .. useThingId .. " SubType: " .. g_game .. setClipboardText(useThingId), function ()
			end)
		else
			menu:addOption("ID: " .. useThingId, function ()
				g_game.setClipboardText(useThingId)
			end)
		end
	end
...
function moveStackableItem(item, toPos)
	...
	local moveFull = modules.client_options.getOption("moveFullStack")
...
function refreshViewMode()
	...
	for i = 1, gameRightPanels:getChildCount() + gameLeftPanels:getChildCount() do
		...
		if gameRightPanels:getChildCount() < i then
	...
	for _, widget in ipairs(gameLeftActions:getChildren()) do
		...
		function widget.onClick()
			if widget.image:isChecked() then
				...
				if widget.doubleClickAction and g_clock.millis() < widget.lastClicked + 200 then
